...
import { boxBorderLR, boxRow } from "./box-chrome.js";
import { registerCompactReadOnlyTools, shortenPath as shortenToolPath } from "../shared/tool-rendering.js";
import {
  composeEditorAutocompleteProvider,
  subscribeEditorAutocompleteContributors,
} from "./editor-capabilities.js";
import { HorizontalLineWidget, WidgetRowRegistry } from "./widget-row.js";
import type { KeybindingsManager } from "@mariozechner/pi-coding-agent";
import type { AssistantMessage, TextContent } from "@mariozechner/pi-ai";
import { execFile } from "node:child_process";
import { promisify } from "node:util";
import { hasToolCost } from "./tool-cost.js";
...
function editorExtension(pi: ExtensionAPI): void {
  ...
  // register read-only tool renderers at extension init so the session/tool registry
  // sees them before any turn starts. doing this in session_start is too late.
  registerCompactReadOnlyTools(pi, process.cwd());

  pi.on("session_start", async (_event, ctx) => {
    // Install again with the real session cwd. Native tools may be registered after
    // extension init, so this makes the compact renderers win at runtime too.
    registerCompactReadOnlyTools(pi, ctx.cwd);

    if (!ctx.hasUI) return;

    // persistent default: keep tool output collapsed unless explicitly expanded
    ...
    function updateBottomLabel() {
      if (!editor) return;
      const cwd = shortenToolPath(ctx.cwd);
