...
export default function (pi: ExtensionAPI) {
	const answerHandler = async (ctx: ExtensionContext) => {
			...
			// Local-only fork: extract questions via regex from the last assistant message.
			// No second LLM call. Patterns recognized:
			//   "1. text"   "1) text"   "Q1: text"   "- text"   inline "...text?"
			const extractionResult: ExtractionResult = (() => {
				const lines = lastAssistantText.split(/\r?\n/);
				const numbered: ExtractedQuestion[] = [];
				const numRe = /^\s*(?:\(?\d+[.)]|Q\d+[:.)]|[-*•])\s+(.+?)\s*$/i;
				for (const line of lines) {
					const m = line.match(numRe);
					if (m) {
						const text = m[1].trim();
						if (text.length > 0) numbered.push({ question: text });
					}
				}
				if (numbered.length > 0) return { questions: numbered };
				const sentenceQs = (lastAssistantText.match(/[^.?!\n]*\?[\s]*/g) ?? [])
					.map((s) => s.trim())
					.filter((s) => s.length > 0)
					.map((q) => ({ question: q }));
				return { questions: sentenceQs };
			})();
