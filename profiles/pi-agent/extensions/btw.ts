...
export default function (pi: ExtensionAPI) {
	...
	async function createSideSession(ctx: ExtensionCommandContext): Promise<SideSessionRuntime | null> {
		...
		const { session } = await createAgentSession({
			...
			tools: createCodingTools(ctx.cwd),
			resourceLoader: createBtwResourceLoader(ctx),
		});

		const seedMessages = buildSeedMessages(ctx, thread);
		if (seedMessages.length > 0) {
			// pi 0.68+: replaceMessages() removed; assigning state.messages copies the array
	...
	// session_start now fires for startup/reload/new/resume/fork (see event.reason),
	// so one handler covers all session-load cases. The old `session_switch` event
	// was removed in pi 0.63; session_tree still fires after the tree overlay closes.
